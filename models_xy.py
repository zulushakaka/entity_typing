...
from model_utils import sort_batch_by_length, SelfAttentiveSum, SimpleDecoder, MultiSimpleDecoder, CNN, MaskDecoder, Mask2PredDecoder
...
class Model(nn.Module):
  def __init__(self, args, answer_num):
    ...
    self.mask_decoder = MaskDecoder(self.output_dim, answer_num)
    # self.mask2pred_decoder = Mask2PredDecoder(answer_num)
  ...
  def sorted_rnn(self, sequences, sequence_lengths, rnn):
    ...
    # print(sorted_inputs)
    # print(sorted_sequence_lengths.data.tolist())
    packed_sequence_input = pack_padded_sequence(sorted_inputs,
                                                 list(map(int, sorted_sequence_lengths.data.tolist())),
                                                 batch_first=True)
  ...
  def define_loss(self, logits, targets, data_type):
    ...
    if torch.sum(gen_target_sum.data) > 0:
      gen_mask = Variable(torch.squeeze(torch.nonzero(torch.min(gen_target_sum.data, comparison_tensor)), dim=1))
    ...
    if torch.sum(fine_target_sum.data) > 0:
      fine_mask = Variable(torch.squeeze(torch.nonzero(torch.min(fine_target_sum.data, comparison_tensor)), dim=1))
    ...
    if not data_type == 'kb':
      ...
      if torch.sum(torch.sum(finer_targets, 1).data) >0:
        finer_mask = Variable(torch.squeeze(torch.nonzero(torch.min(torch.sum(finer_targets, 1).data, comparison_tensor)), dim=1))
  ...
  def forward(self, feed_dict, data_type):
    ...
    logits = self.sigmoid_fn(self.decoder(output, data_type))
    # add mask
    mask, complete_mask = self.mask_decoder(output, logits, data_type)
    masked_logits = logits * (1 - mask)
    # masked_logits = mask
    loss = self.define_loss(masked_logits, feed_dict['y'], data_type)
    # disc_loss = self.descriminator_loss(complete_mask, feed_dict['y'], data_type)
    mask_decy = torch.mean(mask ** 2)
    loss += mask_decy

    return loss, masked_logits, logits, torch.mean(mask)

  # def descriminator_loss(self, mask, target, data_type):
  #   pred = self.mask2pred_decoder(mask, data_type)
  #   loss = self.define_loss(mask, target, data_type)
  #   return loss

  # def disc_parameters(self):
  #   return self.mask2pred_decoder.parameters()

  # def model_parameters(self):
  #   disc_parameters = self.disc_parameters()
  #   return filter(lambda x: x not in disc_parameters, self.parameters())